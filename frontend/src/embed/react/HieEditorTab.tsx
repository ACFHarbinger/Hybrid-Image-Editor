...
export interface HieEditorTabProps {
  showModal: (
    message: string,
...
/**
 * Hybrid Image Editor tab for Image-Toolkit's React/Tauri frontend.
 * Lives in the HIE submodule so host apps pick up editor UI changes on submodule update.
 */
export const HieEditorTab: React.FC<HieEditorTabProps> = ({ showModal }) => {
  const host = useMemo(() => getHieHost(), []);
  ...
  const handleOpenMedia = async () => {
    try {
      ...
      const result = await host.openMedia();
      const openedId =
        result && typeof result === "object" && "documentId" in result
          ? result.documentId
          : undefined;
      if (openedId) {
        setDocumentId(openedId);
      } else {
        // Hosts without IPC document IDs still get a local session label.
        setDocumentId((prev) => prev ?? `local-${Date.now().toString(16)}`);
      }
      setActiveFrame(1);
      setTotalFrames(1);
      setStatusMessage("Media sequence loaded: 1 frame (still image paradigm)");
      setSaveState("MODIFIED");
      host.notify("Media sequence loaded");
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : String(err);
      showModal(`Failed to open media: ${message}`, "error");
    }
  };

  const handleExport = async () => {
    try {
      await host.exportDocument(documentId ?? undefined);
      showModal("Exporting document via HIE render pipeline...", "info", 2500);
      setStatusMessage("Export complete");
      host.notify("Export complete");
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : String(err);
      showModal(`Export failed: ${message}`, "error");
    }
  };

  const handlePreviewAssistance = async () => {
    const active = tools.find((t) => t.id === selectedTool);
    if (!active) return;
    try {
      if (documentId && host.previewPolicy) {
        await host.previewPolicy(documentId, active.policyName);
      }
      setProposalReady(true);
      setStatusMessage(`${active.label} proposal generated — accept to record`);
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : String(err);
      showModal(`Preview failed: ${message}`, "error");
    }
  };
  ...
  const handleAcceptProposal = async () => {
    ...
    if (!active) return;
    try {
      if (documentId && host.acceptProposal) {
        await host.acceptProposal(documentId);
      }
      ...
      const message = err instanceof Error ? err.message : String(err);
      showModal(`Accept failed: ${message}`, "error");
    }
  };
        ...
        <div className="lg:col-span-3 flex flex-col gap-3">
          <div className="relative flex-1 min-h-[420px] bg-gray-950 rounded-xl border border-gray-800 flex flex-col items-center justify-center p-6 text-center overflow-hidden">
            ...
            <div className="relative z-10 max-w-md flex flex-col items-center gap-3">
              ...
              <h3 className="text-base font-semibold text-gray-200">Interactive Canvas Viewport</h3>
              <p className="text-xs text-gray-400 leading-relaxed">
                Drop an image or video sequence here. Single images are processed as a 1-frame
                degenerate sequence paradigm for zero-refactor multi-modal compatibility.
              </p>
          ...
          <div className="flex items-center justify-between gap-3 px-4 py-2 bg-gray-950/60 rounded-lg border border-gray-800 text-xs font-mono">
            <div className="flex items-center gap-2 text-gray-400">
              ...
              <span>
                Frame {String(activeFrame).padStart(2, "0")} / {String(totalFrames).padStart(2, "0")}
              </span>
            </div>
              ...
              {tools.map((tool) => (
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="text-[11px] text-gray-400">{tool.detail}</div>
                  </div>
                </button>
              ))}
            </div>
