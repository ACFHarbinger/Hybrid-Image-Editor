...
| **Phase 3.9** | Standalone Restoration CLI | Med | `middleware/src/restore_cli.py`, `hie-restore` entry point |
...
| **Phase 3.11** | Frontend Restoration Job Dispatch | Med | `middleware/pipeline/restoration.py` and `PipelineSession`, explicit backend capabilities and cancellable preview submission |
