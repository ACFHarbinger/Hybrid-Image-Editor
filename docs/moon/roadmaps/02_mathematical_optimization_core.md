...
| **Phase 2.4** | Python Middleware Optimization Jobs API | High | Cancellable job contract in `middleware/src/hie_middleware/jobs/base.py`, with `exact_dp.py` (`call_hie_exact_solver`) and `metaheuristics.py` (`call_hie_pso`, `call_hie_de`) providing pure-Python reference implementations pending central `base` pybind11 integration (tracked separately — see `hie_claude_handoff_20260812.md`) |
