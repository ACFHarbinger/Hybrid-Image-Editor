...
- `generate_restoration_report` (`middleware/src/hie_middleware/jobs/restoration_report.py`): reusable, independently-testable diagnostic report generator with a real Laplacian-variance sharpness metric (replacing an inline `FIND_EDGES`-filter proxy). `restore_cli.py --report` now uses it.
- `--image PATH` CLI argument for the standalone HIE GUI runner (`python -m hie_gui.main`), loading an image into the viewport and starting a document history for it on startup.
